...
@section('content')
    <section class="relative overflow-hidden bg-[#edf2f7] min-h-screen">
        <!-- ... (gradient shapes remain the same) ... -->
                                ...
                                <div class="space-y-6">
                                    ...
                                    <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                                        @php
                                            ...
                                            // Prepare gallery items with localized descriptions for JS
                                            $galleryForJS = array_map(function ($item) use ($locale) {
    return [
        ...
        'description' => $item['description'][$locale] ?? $item['description']['en'] ?? '',
                                        ...
                                        @foreach($project->images as $index => $item)
                                            @php
                                                $desc = $item['description'][$locale] ?? $item['description']['en'] ?? '';
                                            @endphp
                                            <div class="bg-white/20 backdrop-blur-sm rounded-2xl border border-white/60 overflow-hidden cursor-pointer hover:scale-105 transition shadow-md hover:shadow-xl" onclick="openLightbox({{ $index }})">
                                                ...
                                                @if(!empty($desc))
                                                    <div class="p-3 text-center text-sm text-slate-700 bg-white/30">{{ $desc }}</div>
                                                @endif
                                            </div>
                                        @endforeach
                                    </div>
    ...
    <script>
        // Pass the localized gallery data from PHP
        let galleryItems = @json($galleryForJS ?? []);
        let currentIndex = 0;
